pav_name="../../Pan_genome_data_2/set_minus_orthofinder_result_2/pav_blastn.xlsx"
result_path="../../Pan_genome_data_2/set_minus_orthofinder_result_2/"
library(tidyverse)
library(readxl)
library(data.table)
...
write_class=function(){
  ...
  write.table(
    colnames(fl)[pan_new_bool],file = paste(result_path,"/pan_categories_gene_blastn/","pan_new.txt",sep = ""), row.names = F, col.names = F,append=FALSE,quote = F
  )
}
...
draw_stack=function(){
  ...
  pav_df_cat=merge(pav_df_raw,category,by.x = "...2",by.y = "V1")
