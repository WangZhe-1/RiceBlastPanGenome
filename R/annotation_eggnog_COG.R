...
fun_enrich <- function(gene_taxonomy, annotation_case, levelS) {
  ...
  write.table(enrich_result@result, sprintf("../annotation/add_7_TE_secreted_protein/%s_%s_enrich_result.txt", as.character(substitute(gene_taxonomy)), "cog"), append = F, col.names = T, row.names = F, sep = "\t", quote = F)
...
ggsave("..//picture//annotation_cog_core_10TE.png", plot_core_pathway_C, width = 10, height = 5)
...
ggsave("..//picture//annotation_cog_specific_10TE.png", plot_specific_pathway_C, width = 10, height = 5)