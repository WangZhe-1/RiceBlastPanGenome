...
R_code_filter_strain_num='''
filter_strain_num=function(pav_df_file_name,lower_num,outfile_name){
  require(readxl)
  require(dplyr)
  pav_df=readxl::read_xlsx(pav_df_file_name)
  pav_df_lower_num=pav_df %>% 
    mutate(strain_sum=rowSums(.[3:158])) %>% 
    filter(strain_sum>lower_num) 
  write.table(pav_df_lower_num$Orthogroup,outfile_name,quote = F,row.names = F,col.names = F)
}
'''
...
R_filter_strain_num = SignatureTranslatedAnonymousPackage(R_code_filter_strain_num, "R_filter_strain_num")
...
def ortholog_blast(
    ...
    blast_out_value_tsv_file=(blast_identity_value_dir/(sequence_file_id+".tsv")).resolve()
    sequence_file=(all_row_gene_fasta_dir/(sequence_file_id+".fasta")).resolve()
    if blast_out_value_tsv_file.exists() is True:
        return
    blast_db_file=blast_db_path/(sequence_file_id+"_blast_head_db")
    blast_out_xml_file=blast_out_xml_path/(sequence_file_id+"_blast_out.xml")
    blast_out_asn_file=blast_out_asn_path/(sequence_file_id+"_blast_out.asn")
    blast_out_txt_file=blast_out_txt_path/(sequence_file_id+"_blast_out.txt")
                ...
                if len(record.alignments)>1:
                    ...
                    if MGG_flag:
                        if len(gene_name_list)==0:
                            only_itself_fl.write("{}".format(sequence_file_id))
                            continue
                        ...
                        no_mgg_fl.write("{}".format(sequence_file_id))
                        break
    if rlength(R_blast_vlaue_list)[0]==0:
        length_0_fl.write("{}".format(sequence_file_id))
...
def extract_ortholog_gene(strain_db_dir_path,contig_path,orthogroup_sequence_path,pav_file_path,filter_lower,ortholog_blast_path_name):
    '''
    ...
    filter_lower_list_file=general_out_path/("filter_"+str(filter_lower)+".txt")
    if filter_lower_list_file.exists() is False:
        R_filter_strain_num.filter_strain_num(str(pav_file_path),filter_lower,str(filter_lower_list_file))
    # cluster = LocalCluster()
    # client = Client(cluster)
    calls=[]
    lineList = [line.rstrip('\n') for line in open(filter_lower_list_file,'r+')]
    for all_row_gene_fasta_file in lineList:
        calls.append(dask.delayed(ortholog_blast)(
                all_row_gene_fasta_file.strip(),
                all_row_gene_fasta_dir,
                blast_identity_value_dir,
                only_itself_fl,
                length_0_fl,
...
extract_ortholog_gene(
    ...
    "contig_156_path",
    "/gpfshome/home/Baojd/wangzhe/ortho_blast",
