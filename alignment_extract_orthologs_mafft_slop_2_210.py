...
def ortholog_blast(
    ...
    if blast_out_value_tsv_file.exists() is True:
        return
    blast_db_file=(blast_db_path/(sequence_file.stem+"_blast_head_db")).resolve()
    blast_out_xml_file=(blast_out_xml_path/(sequence_file.stem+"_blast_out.xml")).resolve()
    blast_out_asn_file=(blast_out_asn_path/(sequence_file.stem+"_blast_out.asn")).resolve()
    blast_out_txt_file=(blast_out_txt_path/(sequence_file.stem+"_blast_out.txt")).resolve()
    if blast_out_xml_file.exists() is False:
        blastdb(sequence_file.resolve(),blast_db_file)
        blast(blast_db_file,sequence_file.resolve(),blast_out_xml_file,blast_out_asn_file,blast_out_txt_file)
    MGG_flag=False
    ...
    #     return R_blast_vlaue_df
    # else:
    #     no_mgg_fl.write("{}/n".format(sequence_file.stem))
...
def extract_ortholog_gene(ortholog_blast_path_name):
    ...
    # R_blast_vlaue_list=base.list()
    # i=1
    # for blast_identity_value_file in blast_identity_value_dir:
    #     R_df=utils.read_table(str(blast_identity_value_file),
    #         header = True,
    #         sep = "\t",
