...
pav_df[na_tsv]=1
pav_df[!na_tsv]=0
write.table(pav_df,pav_path,quote = F,sep = "\t",row.names = pav_df_raw$...2)
...
Classify=function(Min,Max,Name){
  ...
  write.table(
    gene_protein$V2,file = paste(result_path,"/pan_categories_protein/",sprintf("%s.txt", Name),sep = ""), row.names = F, col.names = F,append=FALSE,quote = F
  )
  print(c(Min,Max))
  return(core_df)
}
...
pan_new_bool=!str_detect(gene_protein_mapping_table$V1,"MGG")
write.table(
    gene_protein_mapping_table$V1[pan_new_bool],file = paste(result_path,"/pan_categories_gene/","pan_new.txt",sep = ""), row.names = F, col.names = F,append=FALSE,quote = F
  )
write.table(
    gene_protein_mapping_table$V2[pan_new_bool],file = paste(result_path,"/pan_categories_protein/","pan_new.txt",sep = ""), row.names = F, col.names = F,append=FALSE,quote = F
  )
...
"""
def R_run_pav_instance_maker(pav_excel,gene_protein_mapping_table_file_name,R_result,pav_0_1_tsv):
    '''
    input 1: pav_name
    input 2: gene_protein_mapping_table_file_name
    output 1: pav_path_name
    output 2: result_path_name
    ...
    robjects.globalenv["pav_path"]=pav_0_1_tsv
    robjects.globalenv["gene_protein_mapping_table_file_name"]=gene_protein_mapping_table_file_name
...
def set_minus_prepare(start_strain):
    '''
    input 1: indicate which strain is start point,70-15 or Guy11
    5 outputs used in set minus
    '''
    robjects.r['Cut'](start_strain)
def draw_stack():
    '''
    '''
